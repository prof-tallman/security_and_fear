...
def crack(ciphertext):
    """
    Cracks the Caesar Shift Cipher by calculating the decryption key that
    provides the closest letter frequency to standard English. Although there
    is no guarantee this is the correct key, it is most likely solution.
    Returns an encryption key.
    """
    frequency_list = score_all_keys(ciphertext)
    frequency_list = sorted(frequency_list.items(), key=lambda x: x[1])
    best_key = frequency_list[0][0]
